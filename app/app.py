...
import os
import base64
import pickle  # add to imports if not already there
import io
...
# VULN: HardcodedSecret - A02:2021 - Cryptographic Failures
...
# VULN: BrokenAuth - A07:2021 - Broken Authentication
# Hardcoded plaintext admin password seeded into the database on first run.
# No hashing applied. Detected by: Bandit B105/B106, Semgrep auth rules.
...
# VULN: BrokenAuth - A07:2021 - Broken Authentication
# JWT signature verification disabled with verify=False. Any token,
# including forged ones, will be accepted. Used by the /get/<cust_id> route.
# Detected by: Bandit B105, Semgrep jwt rules. Maps to STRIDE threat A-S-001.
...
# VULN: Misconfig - A05:2021 - Server-Side Template Injection (SSTI)
# request.url injected directly into render_template_string() without sanitization.
# Attacker can execute arbitrary Python via Jinja2 template expressions.
# Detected by: Semgrep. Maps to STRIDE threat A-I-002.
    template = '''<html>
    <head>
    <title>Error</title>
    </head>
...
# VULN: BrokenAuth - A07:2021 - Broken Authentication
# Password compared in plaintext against the database with no hashing.
# An attacker with DB read access can immediately use any credential.
# Detected by: Bandit B105/B106, Semgrep auth rules.
...
# VULN: SQLi - A03:2021 - Injection
# User-controlled search_term concatenated directly into SQL query string.
...
# VULN: Deserialization - A08:2021 - Insecure Deserialization
# yaml.load() without Loader argument allows arbitrary Python object
# instantiation from untrusted YAML input. Use yaml.safe_load() instead.
# Detected by: Bandit B506, Semgrep. Maps to STRIDE threat A-T-002.
        ydata = yaml.load(y)

    return render_template('view.html', name = json.dumps(ydata))

# VULN: Deserialization - A08:2021 - Insecure Deserialization
# Accepts raw pickle bytes from the request body and deserializes them
# without any validation. An attacker can craft a malicious pickle payload
# to achieve Remote Code Execution (RCE). Maps to STRIDE threat A-T-002,
# MITRE ATT&CK T1059.006. Detected by: Bandit B301/B302, Semgrep
# python.flask.security.deserialization.
